...
<section class="section">
    <div class="section-header">
        <h3 class="page__heading">Pedido producto</h3>
    </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        {{-- @can('crear-rol')
                        <a href="{{ route('pedido_productos.create') }}" class="btn btn-warning">Nuevo pedido producto</a>
                        @endcan --}}
                        <br>
                        <br>
                        <div class="table-responsive">
                            ...
                            <table class="table table-hover table-striped mt-2" id="table_id">
                                ...
                                <tbody>
                                    @foreach ($pedidosProds as $pedido_producto)
                                    <tr>
                                        <td>{{$pedido_producto->id}}</td>
                                        <td>{{$pedido_producto->producto_id}}</td>
                                        <td>{{$pedido_producto->pedido_id}}</td>
                                        <td>{{$pedido_producto->precio}}</td>
                                        <td>{{$pedido_producto->cantidad}}</td>
                                        {{-- <td>
                                            @can('editar-rol')
                                            <a class="btn btn-info" href="{{ route('pedido_productos.edit', $pedido_producto->id) }}"><i class="fas fa-edit"></i></a>
                                            @endcan
                                            @can('eliminar-rol')
                                            {!! Form::open(['method'=> 'DELETE', 'route'=> ['pedido_productos.destroy', $pedido_producto->id], 'style'=>'display: inline', 'class'=>'formEliminar',]) !!}
